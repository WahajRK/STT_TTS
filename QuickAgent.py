...
class TextToSpeech:
    ...
    def speak(self, text):
        ...
        # # DEEPGRAM_URL = f"https://api.deepgram.com/v1/speak?model=aura-helios-en&performance=true&encoding=linear16&sample_rate=24000"
        ...
        with requests.post(DEEPGRAM_URL, stream=True, headers=headers, json=payload) as r:
            print(f"Status Code: {r.status_code}")  # Debugging line
            if r.status_code != 200:
                print(f"Error: {r.text}")  # Debugging line
            for chunk in r.iter_content(chunk_size=1024):
                if chunk:
                    print("Received chunk")  # Debugging line
